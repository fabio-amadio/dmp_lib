...
def change_frame(trajectory, H_target):
    """Express the trajectory in a new reference frame.

    Parameters
    ----------
    trajectory : numpy.ndarray
        positions and axis-angle [trajectory.shape : (num_step, 6)]
    H_target : numpy.ndarray
        previous-to-new reference frame [H_target.shape : (4, 4)]

    Returns
    ------
    numpy.ndarray
        trajectory expressed in the new reference frame

    Raises
    ------
    ValueError
        If H_target is not a proper hom. tr. matrix
    """
    if H_target.shape != (4,4):
            if not is_rotation_matrix(H_target[:3,:3]):
                if not (H_target[3,0] == H_target[3,1] == H_target[3,2] == 0. 
                    and H_target[3,3] == 1): 
                    raise ValueError('H_target is not a proper \
...
# 2nd-4th col: VF pos / 5th-8th col: VF axis-angle / 9-th col: EE distance
x_demo = demo[:,1:] 
...
tr_dmp = Bimanual_Target_DMP(num_basis=n_basis, tau=tau, dt = dt, x0=x0, g0=g0,
                w_H_t = H_world_to_target, R_right = R_right, R_left = R_left)

tr_x_demo, _, _, tr_demo_time = tr_dmp.learn_from_demo(x_demo, time_steps)
...
wr_dmp = Bimanual_DMP(num_basis=n_basis, tau=tau, dt = dt, x0=x0, g0=g0,
                        R_right = R_right, R_left = R_left)

wr_x_demo, _, _, wr_demo_time = wr_dmp.learn_from_demo(x_demo, time_steps)
...
H_target_to_dmp_goal = np.matmul(np.linalg.inv(H_world_to_target),
                                    H_world_to_dmp_goal)
...
goal_axis, goal_angle = rot_to_axis_angle(H_new_goal[0:3,0:3],
                                            auto_align = True)
...
for k in range(0, tr_trajectory_r.shape[0], int(tr_trajectory_r.shape[0]/5)):
    ...
    ax.quiver(tr_trajectory_r[k,0], tr_trajectory_r[k,1], tr_trajectory_r[k,2],
                R_right[0,0], R_right[1,0], R_right[2,0],
                colors = 'r', length = 0.05, normalize = True)
    ax.quiver(tr_trajectory_r[k,0], tr_trajectory_r[k,1], tr_trajectory_r[k,2],
                R_right[0,1], R_right[1,1], R_right[2,1],
                colors = 'g', length = 0.05, normalize = True)
    ax.quiver(tr_trajectory_r[k,0], tr_trajectory_r[k,1], tr_trajectory_r[k,2],
                R_right[0,2], R_right[1,2], R_right[2,2],
                colors = 'b', length = 0.05, normalize = True)

    R_left = axis_angle_to_rot(tr_trajectory_l[k,3:6])
    ax.quiver(tr_trajectory_l[k,0], tr_trajectory_l[k,1], tr_trajectory_l[k,2], 
                R_left[0,0], R_left[1,0], R_left[2,0],
                colors = 'r', length = 0.05, normalize = True)
    ax.quiver(tr_trajectory_l[k,0], tr_trajectory_l[k,1], tr_trajectory_l[k,2], 
                R_left[0,1], R_left[1,1], R_left[2,1],
                colors = 'g', length = 0.05, normalize = True)
    ax.quiver(tr_trajectory_l[k,0], tr_trajectory_l[k,1], tr_trajectory_l[k,2], 
                R_left[0,2], R_left[1,2], R_left[2,2],
                colors = 'b', length = 0.05, normalize = True)

ax.plot3D(demo_r[:,0], demo_r[:,1], demo_r[:,2], 'tab:orange', linewidth =3,
            label = 'Demo')
ax.plot3D(demo_l[:,0], demo_l[:,1], demo_l[:,2], 'tab:orange', linewidth =3)

ax.plot3D(wr_trajectory_r[:,0], wr_trajectory_r[:,1], wr_trajectory_r[:,2],
            'tab:blue', linewidth = 3,  label = 'WR-DMP')
ax.plot3D(wr_trajectory_l[:,0], wr_trajectory_l[:,1], wr_trajectory_l[:,2],
            'tab:blue', linewidth = 3)

ax.plot3D(tr_trajectory_r[:,0], tr_trajectory_r[:,1], tr_trajectory_r[:,2],
            'tab:green', linewidth = 3,  label = 'TR-DMP')
ax.plot3D(tr_trajectory_l[:,0], tr_trajectory_l[:,1], tr_trajectory_l[:,2],
            'tab:green', linewidth = 3)

for k in range(0, wr_trajectory_r.shape[0], int(wr_trajectory_r.shape[0] / 5)):
    R_right = axis_angle_to_rot(wr_trajectory_r[k,3:6])
    ax.quiver(wr_trajectory_r[k,0], wr_trajectory_r[k,1], wr_trajectory_r[k,2],
                R_right[0,0], R_right[1,0], R_right[2,0],
                colors = 'r', length = 0.05, normalize = True)
    ax.quiver(wr_trajectory_r[k,0], wr_trajectory_r[k,1], wr_trajectory_r[k,2],
                R_right[0,1], R_right[1,1], R_right[2,1],
    ...
    ax.quiver(wr_trajectory_l[k,0], wr_trajectory_l[k,1], wr_trajectory_l[k,2],
                R_left[0,0], R_left[1,0], R_left[2,0],
                colors = 'r', length = 0.05, normalize = True)
    ax.quiver(wr_trajectory_l[k,0], wr_trajectory_l[k,1], wr_trajectory_l[k,2],
                R_left[0,1], R_left[1,1], R_left[2,1],
...
for k in range(0, demo_r.shape[0], int(demo_r.shape[0] / 5)):
    ...
    ax.quiver(demo_r[k,0], demo_r[k,1], demo_r[k,2], R_right[0,0], R_right[1,0],
                R_right[2,0], colors = 'r', length = 0.05, normalize = True)
    ax.quiver(demo_r[k,0], demo_r[k,1], demo_r[k,2], R_right[0,1], R_right[1,1],
                R_right[2,1], colors = 'g', length = 0.05, normalize = True)
    ax.quiver(demo_r[k,0], demo_r[k,1], demo_r[k,2], R_right[0,2], R_right[1,2],
                R_right[2,2], colors = 'b', length = 0.05, normalize = True)
    R_left = axis_angle_to_rot(demo_l[k,3:6])
    ax.quiver(demo_l[k,0], demo_l[k,1], demo_l[k,2], R_left[0,0], R_left[1,0],
                R_left[2,0], colors = 'r', length = 0.05, normalize = True)
    ax.quiver(demo_l[k,0], demo_l[k,1], demo_l[k,2], R_left[0,1], R_left[1,1],
                R_left[2,1], colors = 'g', length = 0.05, normalize = True)
    ax.quiver(demo_l[k,0], demo_l[k,1], demo_l[k,2], R_left[0,2], R_left[1,2],
                R_left[2,2], colors = 'b', length = 0.05, normalize = True)
...
ax.scatter3D(tr_trajectory_r[0,0], tr_trajectory_r[0,1], tr_trajectory_r[0,2], 
                color = 'tab:green', linewidth = 3)
ax.scatter3D(tr_trajectory_l[0,0], tr_trajectory_l[0,1], tr_trajectory_l[0,2], 
                color = 'tab:green', linewidth = 3)
ax.scatter3D(wr_trajectory_r[0,0], wr_trajectory_r[0,1], wr_trajectory_r[0,2], 
                color = 'tab:blue', linewidth = 3)
ax.scatter3D(wr_trajectory_l[0,0], wr_trajectory_l[0,1], wr_trajectory_l[0,2], 
                color = 'tab:blue', linewidth = 3)
ax.scatter3D(demo_r[0,0], demo_r[0,1], demo_r[0,2], color = 'tab:orange', 
                linewidth = 3)
ax.scatter3D(demo_l[0,0], demo_l[0,1], demo_l[0,2], color = 'tab:orange', 
                linewidth = 3)
