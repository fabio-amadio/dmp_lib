...
x_demo = demo[:,1:] # 2nd-4th columns: position / 5th-8th columns: axis-angle
...
x_demo, dx_demo, ddx_demo, time_steps = mp.learn_from_demo(x_demo, time_steps, 'savgol')
...
for k in range(0,x_demo.shape[0],50):
    ...
    ax.quiver(x[k,0], x[k,1], x[k,2], R[0,0], R[1,0], R[2,0], colors='r', length=0.05, normalize=True)
    ax.quiver(x[k,0], x[k,1], x[k,2], R[0,1], R[1,1], R[2,1], colors='g', length=0.05, normalize=True)
    ax.quiver(x[k,0], x[k,1], x[k,2], R[0,2], R[1,2], R[2,2], colors='b', length=0.05, normalize=True)
    # plot trajectories
